...
static int max86900_set_hr_range1(struct max86900_device_data *data)
{
	int err;
	err = max86900_write_reg(data, MAX86900_SPO2_CONFIGURATION,
		data->hr_range1);
	...
	pr_info("%s - hr_range1 = %u\n", __func__, data->hr_range1);
	return 0;
}
...
static void max86900_eol_test_onoff(struct max86900_device_data *data, int onoff)
{
	...
	if (onoff) {
		...
		data->hr_range1 = 0x47; /* set default hr_range 46.9bpm */
		data->hr_range = 0x47;
...
static ssize_t hr_range1_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	int err;
	struct max86900_device_data *data = dev_get_drvdata(dev);

	err = kstrtou8(buf, 10, &data->hr_range1);
	if (err < 0)
		return err;

	err = max86900_set_hr_range1(data);
	if (err < 0)
		return err;
...
static ssize_t hr_range1_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct max86900_device_data *data = dev_get_drvdata(dev);

	pr_info("%s - hr_range1 = %x\n", __func__, data->hr_range1);
...
static DEVICE_ATTR(hr_range1, S_IRUGO | S_IWUSR | S_IWGRP,
	hr_range1_show, hr_range1_store);
